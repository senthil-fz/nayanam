...
export class WeeklySummaryScheduler {
  ...
  async runOnce(): Promise<{ sent: number; skippedLock: boolean; skippedEmpty: number }> {
    return this.prisma.$transaction(async (tx) => {
      ...
      // for this week.
      const candidates = await tx.$queryRaw<Array<{ user_id: string; email: string }>>`
      ...
      for (const c of candidates) {
        try {
          ...
          // user.weekly_summary_sent event into the user's first household.
          const m = await tx.householdMember.findFirst({ where: { userId: c.user_id } });
          if (m?.householdId) {
            await tx.$executeRaw`
              INSERT INTO events (id, household_id, actor_id, type, payload)
              VALUES (
                ${newId()}, ${m.householdId}, NULL, 'user.weekly_summary_sent',
          ...
          this.logger.error(`weekly-summary send failed user_id=${c.user_id} error=${String(err)}`);
        }
      }
